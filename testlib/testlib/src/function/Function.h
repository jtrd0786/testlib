...
#if 0
#pragma mark --- DefaultAllocator_T-Impl
#endif // 0
#if 1
...
public:
	...
	{
		intptr_t new_n = n; // Math::alignTo(n, s_kAlign);
		return new char[new_n];
	}
...
public:
	_NODISCARD void* alloc(SizeType n)
	{
		if (n > s_kLocalSize)
			return nullptr;
		return &_buf;
...
public:
	...
	void free(void* p, SizeType n)
	{
		assert(!isUsingLocalBuffer(p)/*, "cannot free local buffer pointer"*/);
...
private:
	...
	public:
		...
		{ 
			//printf("sizeof(FUNC): %llu\n", sizeof(FUNC));
			//printf("sizeof(Functor): %llu\n", sizeof(Functor));
			//printf("sizeof(_func): %llu\n", sizeof(_func));
